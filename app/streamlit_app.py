...
def main() -> None:
    ...
    with st.expander("What this tool can and can't do yet"):
        st.markdown(
            ...
            "compliance decisions, and works reliably at working distance (a person "
            "reasonably close to and facing the camera).\n"
            ...
            "- **Crowded, wide-shot, or elevated-camera scenes are a known, tested "
            "limitation with two distinct causes.** First, role can't currently be "
            "told apart from a bounding box alone — a passenger or bystander may be "
            "counted as personnel requiring PPE. Second, and more fundamentally, real "
            "vests in dense or distant scenes can produce genuinely lower detection "
            "confidence (tested as low as ~0.45 on real footage) than the threshold "
            "needed to safely avoid false positives elsewhere (a plain shirt was "
            "misread as a vest at 0.44 confidence in earlier testing) — so a lower "
            "threshold isn't a safe fix. In this situation, a compliant person can be "
            "flagged as a violation. This is a real, currently unresolved detection-"
            "quality limit, not a bug in the compliance logic — see "
            "`docs/RESEARCH_LOG.md` (2026-08-13 entries) for the full diagnostic "
            "history and why threshold tuning alone can't close this gap.\n"
